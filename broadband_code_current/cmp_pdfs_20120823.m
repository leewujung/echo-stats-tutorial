...
fname_pre = 'lower_upper_wide_hann';
...
%t{2} = 'no_taper';
t{1} = 'sqchirp_lowerHwide';
t{2} = 'sqchirp_upperHwide';
...
tt{1} = 'lower wide hann win';
tt{2} = 'upper wide hann win';
%tt{3} = 'upper hann win';
%tt{1} = 'narrow Hann';
%tt{2} = 'wide Hann';
%tt{1} = 'without bp';
...
for N = Nscat_all
    ...
    for iF = 1:length(t)
        ...
        eval(['[x,p_s',num2str(iF)','] = findEchoDist(s',num2str(iF),'.s,''default'',0,50);']);
