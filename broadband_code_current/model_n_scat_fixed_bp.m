...
function model_n_scat_fixed_bp(N,sampleN,gateLdist,use_ideal,taper_opt,save_opt,save_dir,fixfreq,fname)
%INPUT
...
%   fixfreq     frequency of the fixed beampattern [Hz]
...
save_file = [fname,'_','N',num2str(N),...
             ...
             '_fixfreq',num2str(fixfreq/1e3),'kHz.mat'];
if taper_opt==1
...
theta = rand_piston_angle(N)';  % angle in the beam
                                % theta = 10/180*pi;
b = (2*besselj(1,k*a*sin(theta))./(k*a*sin(theta))).^2;

resp_1smpl = zeros(1,rL);
for iN=1:N
    resp_temp = amp(iN)*b(iN)*...
...
param.description = sprintf('%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n',...
                            ...
                            'fixfreq   - the freq of which the fixed beampattern is from',...
